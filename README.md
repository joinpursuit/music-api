...
1. Edit the `script` tag in the 'all-songs.html' page (in the 'views' folder) to do the following: When you go to 'localhost:8888/view/all-songs' in your browser (the route is already set up for you in the server.js file), the page should display all the songs in your database as a list. The list should show artist, title, and youtube link (for example, Frank Ocean - Pyramids [YouTube Link](https://www.youtube.com/watch?v=UHy8hKXi2Pc)). As a bonus you can also try to embed the youtube video using the link. You should only edit the `script` tag (and not the actual HTML). You can use either native DOM methods or jQuery.
